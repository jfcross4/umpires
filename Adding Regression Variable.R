...
gd_savant_new_slim <- gd_savant_new_slim %>% 
...
gd_savant_new_slim_top_middle_noswing$pred_strike_logistic <- predict(m_logistic, gd_savant_new_slim_top_middle_noswing,
                                                                    type="response")
intercept_coef <- -1*coef(m_logistic)["(Intercept)"]
...
gd_savant_new_slim <- gd_savant_new_slim  %>% filter(sz_top!=0) %>%
  mutate(pred_top = (intercept_coef + sztop_coef*sz_top + avg_sztop_coef*avg_sz_top +above_height_top_coef*percent_height_top*Height/12)/
...
m <- lm(strike ~ pred_top, data = gd_savant_new_slim_top_middle_noswing)
m_logistic <- glm(strike ~ pred_top, data = gd_savant_new_slim_top_middle_noswing,
                  family = "binomial")