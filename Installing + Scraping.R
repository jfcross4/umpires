...
umpire_full_data %>% group_by(description) %>% summarise(n=n()) %>% arrange(n)

umpire_full_data <- umpire_full_data %>% dplyr::filter(description %in% c("ball", "called_strike"))

umpire_full_data <- umpire_full_data %>% mutate(strike = ifelse(description=="ball", 0, 1))
